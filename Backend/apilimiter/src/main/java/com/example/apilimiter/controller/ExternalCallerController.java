...
public class ExternalCallerController {
    ...
    @GetMapping("/{id}")
public ResponseEntity<?> proxy(@PathVariable("id") String projectshortname){
Project project=projectRepo.findByShortname(projectshortname).orElseThrow(()->new IllegalArgumentException("No such Api-Project found."));
